...
static int gip_drums_op_authenticate(struct gip_client *client,
				       void *data, u32 len)
{
	struct gip_drums *drums = dev_get_drvdata(&client->dev);

	return gip_auth_process_pkt(&drums->auth, data, len);
}
...
static int gip_drums_probe(struct gip_client *client)
{
	...
	err = gip_auth_start_handshake(&drums->auth, client);
	if (err)
		return err;
...
static struct gip_driver gip_drums_driver = {
	...
	.ops = {
		...
		.authenticate = gip_drums_op_authenticate,
